...
int TCPConnect::SignUP(QString userName, QString password)
{
   ...
   /* QByteArray passtemp = password.toUtf8();
    QCA::Hash hashObj("SHA256");
    password = hashObj.hashToString(passtemp);
    ...
    //qDebug()<<regString.toLocal8Bit();
    socket->write(regString.toLocal8Bit());
...
int TCPConnect::LogOut()
{
    ...
    {
        QMessageBox::information(this,"Error","Logout unsuccessful");
        return 0;
    }
}
...
int TCPConnect::fetchCurrentStreak()
{
    QString temp;
    socket->write("CURSTR");
    if(!socket->waitForBytesWritten(3000))
